...
// private $show_mail_receipt = true;
    ...
    */
    ...
    public function __construct($form_id, $post_id, $hidden = array(), $type = 'request', $args = array(), $_post = null ) {// , $sections, $settings = false  //"option") {
        ...
        $this->html_builder = new WP_Swift_Form_Builder_Html($form_data, $form_id, $post_id, $hidden = array(), $type = 'request', $args = array(), $_post = null);
    ...
    public function run() {
        // echo '<pre>run run run: '; var_dump('run run run'); echo '</pre>';
        // echo '<pre>$this->form_data: '; var_dump($this->form_data); echo '</pre>';
        if ($this->form_data) {
            // return $this->get_form();
            return $this->html_builder->run($this);
        }
        // else {
    ...
    public function get_form_response($button_name) {
        $html_response = '';
  ...
  /*
    ...
    public function process_form($post, $ajax=false) {
        ...
        if ( $this->recaptcha_check($post) && $this->get_form_data() ) {
            ...
            echo '<pre>$this->html_builder->get_form_pristine() '; var_dump($this->html_builder->get_form_pristine()); echo '</pre>';
